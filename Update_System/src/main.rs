...
fn main() -> io::Result<()> {
    // Check if running inside Alacritty
    let is_alacritty = std::env::var("TERM").unwrap_or_default().contains("alacritty");

    // If not running in Alacritty, spawn Alacritty and exit
    if !is_alacritty {
        let current_exe = std::env::current_exe()?;
        let mut alacritty_process = Command::new("alacritty")
        .arg("-e")
        .arg(current_exe)
        .spawn()?;

        // Store the process ID for later termination
        let alacritty_pid = alacritty_process.id();
        std::env::set_var("ALACRITTY_PID", alacritty_pid.to_string());

        // Wait for Alacritty to finish
        alacritty_process.wait()?;
        return Ok(());
    }
        ...
        if let Event::Key(key_event) = event::read()? {
            match key_event.code {
                KeyCode::Char('e') | KeyCode::Char('E') => {
                    ...
                    // Terminate Alacritty process
                    if let Ok(pid_str) = std::env::var("ALACRITTY_PID") {
